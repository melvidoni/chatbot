...
public class Search extends Solve {
    ...
    public SearchAction solve(Object[] params) 
    {
        ...
        while (searchStrategy.getNodesToExpandSize() > 0 & !goal) 
        {
            ...
            System.out.println("Operador: " + firstNode.getAction());
            System.out.println("Estado: \n" + firstNode.getAgentState().toString());
            ...
            {	// If the actual node is not a goal node then it must be expanded.-
                ...
                for (int i = 0; i < actionList.size(); i++) 
                {
                    ...
                    if (ast != null) 
                    {	// If the action was correctly executed.-
        ...
        //ac· si fija si llegÛ al objetivo Y crea un camino con todos los
        //operadores que fue aplicando para que asÌ sea. 
        //en la posiciÛn 0 de path, se encuentra el operador que 
        //aplicÛ ˙ltimo
        if (goal && !getBestPath().isEmpty()) 
        {
            // This variable store the branch's path where the node belongs.-
            Vector<NTree> path = getBestPath();
            System.out.println("Mejor camino: " + path);
    ...
    public Vector<NTree> getBestPath() {
